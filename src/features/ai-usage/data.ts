import type { AiUsageResponse, MachineUsageDay, UsageTotals } from "@/features/ai-usage/types";
...
export function buildUsageModel(response: AiUsageResponse) {
  const sourceCount = response.expectedSourceCount
    || new Set([...(response.expectedMachineIds || []), ...response.machines.map((machine) => machine.machineId)]).size
    || 1;
  ...
  const daily = datesBetween(start, today).map<DailyUsage>((date) => {
    ...
    for (const machine of response.machines) {
      ...
      for (const agent of day.agents) {
        ...
        else if (agent.agent === "codex") item.codex += agent.totalTokens;
        else if (agent.agent === "hermes") item.hermes += agent.totalTokens;
  ...
  return { daily, latest, reportingYear, sourceCount, today, yearTotals };
}
