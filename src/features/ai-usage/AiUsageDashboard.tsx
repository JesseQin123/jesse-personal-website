...
import type { UsageTotals } from "@/features/ai-usage/types";
...
function ActivityCalendar({ days, sourceCount, today, year }: { days: DailyUsage[]; sourceCount: number; today: string; year: string }) {
  ...
  const latestObservedDate = observedDays.at(-1)?.date;
  const yearStart = `${year}-01-01`;
  const gridStart = shiftDate(yearStart, -new Date(`${yearStart}T12:00:00Z`).getUTCDay());
  const gridEndBase = `${year}-12-31`;
  ...
  const colorFor = (date: string, day?: DailyUsage) => {
    if (date > today || !date.startsWith(year)) return "transparent";
  ...
  useEffect(() => {
    if ((!selected || selected.coverage === 0) && latestObservedDate) setSelectedDate(latestObservedDate);
  }, [latestObservedDate, selected]);

  const moveSelection = (offset: number) => {
    const nextDate = shiftDate(selectedDate, offset);
    if (!nextDate.startsWith(year) || nextDate > today) return;
    setSelectedDate(nextDate);
    requestAnimationFrame(() => gridRef.current?.querySelector<HTMLButtonElement>(`[data-date="${nextDate}"]`)?.focus());
  };

  const handleCalendarKeyDown = (event: KeyboardEvent<HTMLButtonElement>) => {
  ...
  return (
    <Card className="mt-8 border-border/80 shadow-sm">
      <CardHeader className="gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div><CardTitle className="text-xl">{year} daily activity</CardTitle><p className="mt-2 text-sm text-muted-foreground">Select a day, or use arrow keys, to inspect Jesse's aggregated usage.</p></div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="rounded-lg bg-[#071827] px-4 py-2 text-sm font-bold text-white">{year}</span>
        </div>
      </CardHeader>
      <CardContent>
        <div className="overflow-x-auto pb-3">
          <div ref={gridRef} className="min-w-[1040px]" role="group" aria-label={`${year} AI usage activity`} style={{ display: "grid", gridTemplateColumns: `44px repeat(${weekCount}, 14px)`, gridTemplateRows: "22px repeat(7, 14px)", gap: "4px" }}>
...
export default function AiUsageDashboard() {
  ...
  const liveStatus = query.isError ? "UNAVAILABLE" : query.isPending ? "LOADING" : !hasReportedData ? "WAITING" : isFresh ? "LIVE" : "STALE";
  const statusIsHealthy = liveStatus === "LIVE";
          ...
          <div className="container relative mx-auto px-4 py-14 lg:px-8 lg:py-20">
            <div className="grid items-end gap-10 lg:grid-cols-[1.2fr_0.8fr]">
              <div>
                ...
                <p className="mt-6 max-w-2xl text-lg leading-8 text-muted-foreground">A transparent daily record of Jesse's AI-assisted workflows—measured in tokens processed, fresh input/output, cache reuse, and estimated cost.</p>
              </div>
              <div className="rounded-2xl border border-border bg-background/90 p-6 shadow-xl backdrop-blur-sm lg:p-8">
                <div className="flex items-center justify-between"><span className="text-sm font-medium text-muted-foreground">Latest observed day</span><span className={`flex items-center gap-1.5 text-xs font-semibold ${statusIsHealthy ? "text-emerald-700" : "text-amber-700"}`}><span className={`h-2 w-2 rounded-full ${statusIsHealthy ? "bg-emerald-500" : "bg-amber-500"}`} />{liveStatus}</span></div>
        ...
        <section className="container mx-auto px-4 py-10 lg:px-8 lg:py-14">
          ...
          <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.55fr)]">
            <Card className="border-border/80 shadow-sm">
              <CardHeader><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-primary">Last 30 days</p><CardTitle className="mt-2 text-2xl">Daily token throughput</CardTitle></div><div className="flex flex-wrap gap-4 text-xs text-muted-foreground"><span><i className="mr-1.5 inline-block h-2 w-2 bg-[#111827]" />Codex</span><span><i className="mr-1.5 inline-block h-2 w-2 bg-[#f97316]" />Claude</span><span><i className="mr-1.5 inline-block h-2 w-2 bg-[#0d9488]" />Hermes</span><span><i className="mr-1.5 inline-block h-2 w-2 bg-[#94a3b8]" />Other</span></div></div></CardHeader>
              <CardContent><div className="h-[360px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={recent} barCategoryGap="20%"><CartesianGrid stroke="hsl(var(--border))" strokeDasharray="3 4" vertical={false} /><XAxis axisLine={false} dataKey="date" fontSize={11} tickFormatter={shortDate} tickLine={false} /><YAxis axisLine={false} fontSize={11} tickFormatter={compact} tickLine={false} width={48} /><Tooltip formatter={(value: number) => compact(value)} labelFormatter={(label) => shortDate(String(label))} /><Bar dataKey="codex" fill="#111827" isAnimationActive={false} stackId="tokens" /><Bar dataKey="claude" fill="#f97316" isAnimationActive={false} stackId="tokens" /><Bar dataKey="hermes" fill="#0d9488" isAnimationActive={false} stackId="tokens" /><Bar dataKey="other" fill="#94a3b8" isAnimationActive={false} stackId="tokens" /></BarChart></ResponsiveContainer></div></CardContent>
            </Card>

            <Card className="border-border/80 bg-foreground text-background shadow-sm">
              <CardHeader><p className="text-xs font-semibold uppercase tracking-wider text-primary">Year to date</p><CardTitle className="text-2xl text-background">{model.reportingYear} activity</CardTitle></CardHeader>
              <CardContent className="space-y-7">
                ...
                <div className="rounded-xl bg-background/10 p-4 text-sm leading-6 text-background/70"><CheckCircle2 className="mb-2 h-4 w-4 text-primary" />Only usage metadata is uploaded. Machine-level details remain private; the public API publishes aggregates.</div>
              </CardContent>
            </Card>
          </div>
        </section>
